...
import { commonSaveBrowserCompat, tsCommonSaveBrowserCompat } from './common-save-compat.mjs';
...
import { galleryUnlockBrowserCompat, subFolderPictureBrowserCompat, UNLOCK_FILE_NAMES } from './gallery-unlock-compat.mjs';
...
import { mtoolTranslatorFastCompat } from './mtool-translator-fast.mjs';
...
// mist: _by_[a-z0-9_]+ = BY_GlobalData 等 BY 系插件全局档（Loccubus v1.0：400 → CG 解锁记录读不到、回想内计数全 0）；master = NovelGameUI 跳过标记主档。
const SAVE_NAME = /^(global|config|common|shared|uta_common|rngd_recomode_switch|ntretdRecoUnlock|anotherNewGame|achievement[s]?|titleInfo|GlobalData|drill_global|drill_timer|gameEnd|gameRecall|gameCloth|trueEnd|file\d+|autosave|MAkaisouSlumguide|knsGlobalInfo|saveimg\d+|_by_[a-z0-9_]+|master)$/;
...
function nwGuiTailGuardCompat(source){
  // mist: 汉化/整合组在 main.js 末尾追加 NW.js 工具条（IIFE 内 require('nw.gui')，例：贪狼游戏合集）。
  // 浏览器无 require → 同步 ReferenceError → main.js onWindowError 记录 → window load 时跳过
  // SceneManager.run → 黑屏。仅当确认处于 NW.js（require 与 nw 同时存在）才执行该块；浏览器直接跳过。
  return source.replace(/(\(function\s*\(\)\s*\{\r?\n)([ \t]*)(var\s+gui\s*=\s*require\(\s*(['"])nw\.gui\4\s*\);)/g, '$1$2if(typeof require!=="function"||typeof nw==="undefined")return; /* mist: 浏览器跳过 NW.js 工具条 */\n$2$3');
}
async function serveMainCompat(req,res,gameRoot,gameId,loadSession) {
  ...
  let output=nwGuiTailGuardCompat(hirokaMainCompat(await compatMatches(game,'hiroka'),versioned));
...
async function serveMtoolFastCompat(req,res,gameRoot,loadSession) {
  // mist: mtool_translator 的数万键巨型正则让手机 WebView 首段翻译卡死数十秒~数分钟
  // (单次 replace 在 V8 解释器路径上逐位置×数万备选扫描)。手机 UA 追加覆写块换分桶匹配器，
  // 桌面 UA 原样直出零改动。语义等价由 tests/mtool-translator-fast.mjs 对拍保证。
  const relative='/js/plugins/mtool_translator.js',file=path.join(gameRoot,...relative.slice(1).split('/')),source=await fsp.readFile(file,'utf8').catch(()=>null);if(source===null)return text(res,404,'文件不存在');
  const output=isMobileUserAgent(req.headers['user-agent']||'')?mtoolTranslatorFastCompat(source):source;
  if(output!==source)console.log('[雾灯兼容] mtool 快速匹配器已注入: mtool_translator.js');
  return serveCompatBuffer(req,res,Buffer.from(output),'text/javascript; charset=utf-8',relative,loadSession);
}
async function serveTSCommonSaveCompat(req,res,gameRoot,loadSession) {
  const relative='/js/plugins/TS_CommonSave.js',file=path.join(gameRoot,...relative.slice(1).split('/')),source=await fsp.readFile(file,'utf8').catch(()=>null);if(source===null)return text(res,404,'文件不存在');
  const output=tsCommonSaveBrowserCompat(source);
  return serveCompatBuffer(req,res,Buffer.from(output),'text/javascript; charset=utf-8',relative,loadSession);
}
async function serveGalleryUnlockCompat(req,res,gameRoot,gameId,name,loadSession) {
  ...
  const output=name==='SubFolderPicture'?subFolderPictureBrowserCompat(source,gameId):galleryUnlockBrowserCompat(source,gameId,name);
  return serveCompatBuffer(req,res,Buffer.from(output),'text/javascript; charset=utf-8',relative,loadSession);
}
...
async function serveEncryptedAudioM4a(req,res,root,sub,game,loadSession){
  ...
  let key=await loadEncryptionKey(root);
  if(!key){ const keyRoot=await resourceRootFor(game,'/data/System.json',root); if(keyRoot&&keyRoot!==root) key=await loadEncryptionKey(keyRoot); }
...
// mist: ヤレるチケット(ed4734b8) 的 JsScript64Set.js（Thirop ParticleEditor 开发工具）与 76
// 字节级同一坏块（帮助模板 2 处未闭合字符串）→ 整个插件 SyntaxError。复用同一修复器；
// 插件本体在三重 isNwjs/test 守卫后惰性，浏览器侧修复不引入行为差。
async function serveJsScript64SetCompat(req,res,gameRoot,loadSession){
  const file=path.join(gameRoot,'js','plugins','JsScript64Set.js');
  const source=await fsp.readFile(file,'utf8').catch(()=>null);
  if(source===null)return text(res,404,'文件不存在');
  const stripped=source.charCodeAt(0)===0xFEFF?source.slice(1):source;
  const output=Buffer.from(fixJsScript76Set(stripped),'utf8');
  res.writeHead(200,{'Content-Type':'text/javascript; charset=utf-8','Content-Length':output.length,'Cache-Control':'no-store'});
  return res.end(output);
}
// mist: PicturePointColor.js（ピクチャ取色）—— 画像読み込み中の Bitmap（0×0）や空名
// （bitmap null）で NaN/Infinity 座標 → 現代 Chromium は getImageData(NaN,…) が
// TypeError "Value is not of type 'long'." → 事件解译器异常 → 场景停止（回想报错）。
// 修正: 几何/bitmap 守卫返回 -1（ピクチャ外扱い）。读取完成后自动恢复正常取色。
async function servePicturePointColorCompat(req,res,gameRoot,loadSession){
  const file=path.join(gameRoot,'js','plugins','PicturePointColor.js');
  const source=await fsp.readFile(file,'utf8').catch(()=>null);
  if(source===null)return text(res,404,'文件不存在');
  const {source:fixed,changed}=fixPicturePointColor(source.replace(/^\uFEFF/,''));
  if(changed)console.log('[雾灯兼容] PicturePointColor.js 取色几何守卫已注入');
  const output=Buffer.from(fixed,'utf8');
  res.writeHead(200,{'Content-Type':'text/javascript; charset=utf-8','Content-Length':output.length,'Cache-Control':'no-store'});
  return res.end(output);
}
...
function ramzaSplashPathCompat(source) {
  // mist: splash 参数 File 常带前导斜杠（如 /img/system/Splash_saikey，作者文档示例如此）→ 插件把
  // folder 拼成 '/img/system/'。NW.js 里 App 根=游戏根，绝对路径可解析；浏览器挂在 /games/<id>/ 下
  // → 请求落到柜根 /img/... → 404 → 位图永不就绪 → Scene_Boot 卡死（且重复重试）。去掉 folder
  // 前导斜杠 → 走相对路径（仅改服务出件，NW 原文件不受影响；参数无前导斜杠的游戏为无操作替换）。
  if(source.includes('/* mist:splash-path */'))return source;
  if(!(/Ramza\.PTS\.params\.splashList/.test(source)&&/this\[index\]\.File\.join\('\/'\) \+ "\/"/.test(source)))return source;
  return source.replace(/this\[index\]\.File\.join\('\/'\) \+ "\/"/, `(this[index].File.join('/') + "/").replace(/^\\/+/, "")/* mist:splash-path */`);
}
async function serveRamzaSplashCompat(req,res,gameRoot,loadSession) {
  const relative='/js/plugins/Ramza_PreTitleSplash_MZ.js',file=path.join(gameRoot,...relative.slice(1).split('/')),source=await fsp.readFile(file,'utf8').catch(()=>null);
  if(source===null)return text(res,404,'文件不存在');
  return serveCompatBuffer(req,res,Buffer.from(ramzaSplashPathCompat(source)),'text/javascript; charset=utf-8',relative,loadSession);
}
async function serveChimakiLangCompat(req,res,gameRoot,loadSession) {
  const relative='/js/plugins/Chimaki_Lang.js',file=path.join(gameRoot,...relative.slice(1).split('/')),source=await fsp.readFile(file,'utf8').catch(()=>null);
  if(source===null)return text(res,404,'文件不存在');
  const output=chimakiLangFsCompat(source);
  return serveCompatBuffer(req,res,Buffer.from(output),'text/javascript; charset=utf-8',relative,loadSession);
}
...
async function serveCoreWithCompat(req,res,gameRoot,fileName,loadSession,gameId) {
  ...
  let coreText=source.toString('utf8'),wlDirect=null;
  if(isMobileUserAgent(req.headers['user-agent']||'')){wlDirect=windowLayerDirectCompat(coreText);if(wlDirect){coreText=wlDirect;console.log('[雾灯兼容] WindowLayer 直绘已注入: '+fileName+' '+gameId)}}
  const output=Buffer.concat([Buffer.from(encryptedImageHttpCompat(coreText)),Buffer.from(weakNetworkCompat(gameId,reportedFlags)),Buffer.from(fossilStretchCompat(fossil)),Buffer.from(hirokaEncryptedUrlCompat(hiroka)),Buffer.from(eliDecryptCompat(eli)),Buffer.from(itankiLocaleCompat(itanki)),Buffer.from(nwjsGlobalShimCompat(nwjs)),...(wlDirect?[Buffer.from(windowLayerDirectFlag())]:[]),Buffer.from(legacyStringPolyfillCompat())]),headers={'Content-Type':'text/javascript; charset=utf-8','Content-Length':output.length,'Cache-Control':'no-store'},tracker=trackLoadStream(req,res,loadSession,'/js/'+fileName,output.length);res.writeHead(200,headers);tracker.chunk(output.length);res.end(output);
}
...
if(p==='/api/mv-prosave-unwrap'&&req.method==='POST'){const raw=await readRawBody(req);const prefix='@@__FMOO_PROSAVE__@@',bodyText=raw.toString('utf8');if(!bodyText.startsWith(prefix))return text(res,400,'不是 ProSave 格式');try{const json=zlib.inflateRawSync(Buffer.from(bodyText.slice(prefix.length),'base64'));res.writeHead(200,{'Content-Type':'text/plain; charset=utf-8','Content-Length':json.length,'Cache-Control':'no-store'});return res.end(json)}catch(e){return text(res,400,'ProSave 解压失败')}}
    ...
    m=p.match(/^\/api\/game-unlocks\/([^/]+)\/([A-Za-z0-9_]+\.json)$/); if(m){
      const game=games.find(g=>g.id===m[1]&&g.type==='game');if(!game)return json(res,404,{error:'游戏不存在'});
      const name=decodeURIComponent(m[2]);
      if(!UNLOCK_FILE_NAMES.has(name))return json(res,403,{error:'名称不允许'});
      const root=await gameRootFor(game),file=path.join(root,'save',name);
      if(req.method==='GET'){const raw=await fsp.readFile(file,'utf8').catch(()=>null);if(raw===null)return json(res,404,{error:'解锁文件不存在'});
        res.writeHead(200,{'Content-Type':'application/json; charset=utf-8','Content-Length':Buffer.byteLength(raw),'Cache-Control':'no-store'});return res.end(raw);}
      if(req.method==='PUT'){const raw=await readRawBody(req);let parsed;try{parsed=JSON.parse(raw.toString('utf8')||'{}')}catch{return json(res,400,{error:'JSON 无效'})}
        await fsp.mkdir(path.join(root,'save'),{recursive:true}).catch(()=>{});await fsp.writeFile(file,JSON.stringify(parsed),'utf8');return json(res,200,{ok:true,keys:Object.keys(parsed).length});}
    }
    m=p.match(/^\/api\/picture-dir\/([^/]+)\/([^/]+)$/); if(m&&req.method==='GET'){
      const game=games.find(g=>g.id===m[1]&&g.type==='game');if(!game)return json(res,404,{error:'游戏不存在'});
      let folder;try{folder=decodeURIComponent(m[2])}catch{return json(res,400,{error:'目录名无效'})}
      if(!/^[A-Za-z0-9_\-]+$/.test(folder))return json(res,400,{error:'目录名无效'});
      const root=await gameRootFor(game),dir=path.join(root,'img','pictures',folder);
      const entries=await fsp.readdir(dir).catch(()=>[]);
      // 过滤 png/jpg/jpeg/rpgmvp + 排序 + 剥扩展名 + 同名去重（对齐 CGGallery / SubFolderPicture 原语义超集）
      const files=[],seen=new Set();for(const f of entries.filter(f=>/\.(?:png|jpe?g|rpgmvp)$/i.test(f)).sort()){const b=f.replace(/\.(?:png|jpe?g|rpgmvp)$/i,'');if(seen.has(b))continue;seen.add(b);files.push(folder+'/'+b)}
    ...
    m=p.match(/^\/games\/([^/]+)(\/.*)?$/); if(m){
      ...
      if(sub==='/js/main.js')return await serveMainCompat(req,res,root,game.id,loadSession);if(sub==='/js/plugins.js')return await servePluginListCompat(req,res,root,loadSession,game.id);if(sub==='/js/plugins/TS_CommonSave.js')return await serveTSCommonSaveCompat(req,res,root,loadSession);if(drsBoost&&sub==='/js/plugins/DRS_BoostEngine.js')return await serveDrsBoostEngineCompat(req,res,root,game.id,loadSession);if(nwjs&&sub==='/js/plugins/DRS_BoostEngineMV.js')return await serveDrsBoostEngineCompat(req,res,root,game.id,loadSession,'DRS_BoostEngineMV.js');if(hiroka&&new RegExp(`^/js/plugins/(?:${HIROKA_PLUGIN_DIR}/)?(?:PluginUtils|AsyncLoadImage|AdvExtention|FontDictionary)\\.js$`).test(sub))return await serveHirokaPluginCompat(req,res,root,game.id,path.basename(sub,'.js'),loadSession);if(/^\/js\/plugins\/(?:Text2Frame|del_Text2Frame|del_CBR_LoadAllMapFile)\.js$/.test(sub))return await serveText2FrameCompat(req,res,root,path.basename(sub,'.js'),loadSession);if(sub==='/js/plugins/CommonSave.js')return await serveCommonSaveCompat(req,res,root,loadSession);if(/^\/js\/plugins\/(?:TS_ReplayMode|TS_Decode)\.js$/.test(sub))return await serveScenarioPluginCompat(req,res,root,path.basename(sub,'.js'),loadSession);if(/^\/js\/plugins\/(?:TitleExtra|CGGallery|EventGallery|SubFolderPicture)\.js$/.test(sub))return await serveGalleryUnlockCompat(req,res,root,game.id,path.basename(sub,'.js'),loadSession);if(sub==='/js/plugins/CsvAutoLoader.js')return await serveCsvAutoLoaderCompat(req,res,root,loadSession);if(sub==='/js/plugins/mtool_translator.js')return await serveMtoolFastCompat(req,res,root,loadSession);if(sub==='/lng.txt'){const lngSrc=await fsp.readFile(path.join(root,'..','lng.txt')).catch(()=>null)||await fsp.readFile(path.join(root,'lng.txt')).catch(()=>null);if(!lngSrc)return text(res,404,'文件不存在');res.writeHead(200,{'Content-Type':'text/plain; charset=utf-8','Content-Length':lngSrc.length,'Cache-Control':'no-store'});return res.end(lngSrc)}if(sub==='/js/plugins/JsScript76Set.js')return await serveJsScript76SetCompat(req,res,root,loadSession);if(sub==='/js/plugins/JsScript64Set.js')return await serveJsScript64SetCompat(req,res,root,loadSession);if(sub==='/js/plugins/PicturePointColor.js')return await servePicturePointColorCompat(req,res,root,loadSession);if(sub==='/js/plugins/SaveSlotCustom.js')return await serveSaveSlotCustomCompat(req,res,root,loadSession);if(sub==='/js/plugins/DRS_AllDataExtractor.js')return await serveDevExtractorCompat(req,res,root,loadSession);if(sub==='/js/plugins/Movie.js')return await serveMovieFPluginCompat(req,res,root,loadSession);if(sub==='/js/plugins/Sakura_MapNameExtend.js')return await serveSakuraMapNameCompat(req,res,root,loadSession);if(sub==='/js/plugins/SoR_DataNoteExtension_MZ.js')return await serveSoRDataNoteCompat(req,res,root,loadSession);if(sub==='/js/plugins/MoviePicture.js')return await serveMoviePictureCompat(req,res,root,loadSession);if(sub==='/js/plugins/Ayatam_AdvancedCharacterGraphics.js')return await serveAyatamCompat(req,res,root,baseRoot,loadSession);if(sub==='/js/plugins/enc_lv2d.js')return await serveEncLv2dCompat(req,res,root,loadSession);if(itanki&&(sub==='/js/plugins/DKTools.js'||sub==='/js/plugins/DKTools.v5.js'))return await serveItankiDKToolsCompat(req,res,root,sub,loadSession);if(itanki&&sub==='/js/plugins/live2d/cubism4ForMZ.js')return await serveItankiCubismCompat(req,res,root,loadSession);if(sub==='/js/plugins/FlowerCore.js'&&flowerCore)return await serveFlowerCoreCompat(req,res,root,loadSession);if(sub==='/js/plugins/Wataridori_AddFileSystem.js'&&wataridori)return await serveWataridoriCompat(req,res,root,loadSession);if(sub==='/js/plugins/Torigoya_Achievement2.js'&&wataridori)return await serveTorigoyaAchievementCompat(req,res,root,loadSession);if(sub==='/js/plugins/NekoGakuen_SteamworksAPI.js'&&steamworks)return await serveSteamworksCompat(req,res,root,loadSession);if(sub==='/js/plugins/Ramza_PreTitleSplash_MZ.js')return await serveRamzaSplashCompat(req,res,root,loadSession);if(sub==='/js/plugins/Chimaki_Lang.js')return await serveChimakiLangCompat(req,res,root,loadSession);if(sub==='/js/plugins/villaA_AnimationTitleButtonFilter1.js')return await serveVillaAAnimationTitleCompat(req,res,root,loadSession);if(sub==='/js/plugins/CGMZ_SplashScreen.js')return await serveCycloneSplashCompat(req,res,root,loadSession);if(sub==='/js/plugins/stwv_allDataFlag.js')return await serveStwvAllDataFlagCompat(req,res,root,loadSession);if(sub==='/data/ExternMessage_en.csv'&&externMessage)return await serveExternMessageCsvCompat(req,res,root,loadSession);if(sub==='/data/CommonEvents.json')return await serveCommonEventsCompat(req,res,root,loadSession);if(sub==='/js/rpg_core.js'||sub==='/js/rmmz_core.js')return await serveCoreWithCompat(req,res,root,path.basename(sub),loadSession,game.id);if(hiroka&&/\.(?:png_?|ogg_?)$/i.test(sub)){const r=await serveHirokaEncryptedCompat(req,res,root,sub,loadSession);if(r!==null)return r}if(eli){const r=await serveEliPictureCompat(req,res,root,sub,loadSession);if(r!==null)return r}if(itanki){const r=await serveItankiStampCompat(req,res,sub,loadSession);if(r!==null)return r;const r2=await serveItankiLocaleCompat(req,res,root,sub,loadSession);if(r2!==null)return r2}if(sub.startsWith('/img/')){const at=await serveAtlasCompat(req,res,root,sub);if(at!==null)return at;const r=await serveImageTextureCompat(req,res,root,sub);if(r!==null)return r;const r2=await serveMissingImageFallback(req,res,root,sub);if(r2!==null)return r2}if(/\.m4a$/i.test(sub)&&await serveM4aFallback(req,res,root,sub,game.id,loadSession))return;if(/\.rpgmvm$/i.test(sub)&&await serveEncryptedAudioM4a(req,res,root,sub,game,loadSession))return;if(/^\/js\/plugins\/.+\.js$/i.test(sub)){const r=await servePluginMarkerStripCompat(req,res,root,sub,loadSession);if(r!==null)return r}return await serveFile(req,res,root,sub,'public, max-age=86400',loadSession);
    }
