...
self.addEventListener('fetch', function (event) {
    ...
    var hasRange = request.headers.has('range');
    var isDynamicGameArchive = isSameOrigin &&
        url.pathname.indexOf('/api/krkr/') !== -1 &&
        /\.zip$/i.test(url.pathname);

    /* KRKR game.zip is a virtual, read-only archive.  Its bytes are produced
     * from NAS files and every Range request is significant.  Cache API
     * matching ignores Range in common Chromium paths, so cache-first here can
     * return an unrelated 206 (or an empty aborted response) for the next
     * central-directory read. */
    if (hasRange || isDynamicGameArchive) {
